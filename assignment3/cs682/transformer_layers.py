...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # Create the positional encoding matrix with correct dimensions
      
        
        position = torch.arange(0, max_len)
        position = position.reshape(-1, 1)
        div = torch.pow(torch.tensor([1e-4]), torch.arange(0, embed_dim, 2)/embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(self.emd_dim / self.n_head)

        # Apply attention mask
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask == 0, -float('inf'))

        attention = self.attn_drop(F.softmax(scores, -1))

        weighted_sum = torch.matmul(attention, value)

        #reshape the tensor back to (N, S, E)
        weighted_sum = weighted_sum.transpose(1, 2).contiguous().view(N, S, E)

        #linear transformation on the heads
